/*
 * Custom Audio HAL for XIAO ESP32-S3-Sense
 * 基于I2S PDM麦克风的简化实现
 */
...
esp_err_t custom_i2s_mic_init(uint32_t sample_rate, uint8_t bit_depth, uint8_t channels, uint32_t buffer_size)
{
    ...
    // 保存配置参数
    g_audio_device.sample_rate = sample_rate;
    g_audio_device.bit_depth = bit_depth;
    g_audio_device.channels = channels;
    ...
    ESP_LOGI(TAG, "PDM pins: CLK=%d, DIN=%d", PDM_CLK_IO, PDM_DIN_IO);
    return ESP_OK;
}
...
audio_board_handle_t custom_av_stream_audio_init(void *ctx, void *arg, av_stream_hal_config_t *config)
{
    ...
    // // 初始化PDM麦克风 - 使用固定参数
    // esp_err_t ret = custom_i2s_mic_init(16000, 16, 1, 320);
    // 初始化PDM麦克风 - 使用配置中的采样率
    esp_err_t ret = custom_i2s_mic_init(config->audio_samplerate, 16, 1, config->audio_framesize * 2);
    if (ret != ESP_OK) {
